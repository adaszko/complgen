...
def get_autoloaded_completion_output(
    ...
    if pid == 0:
        ...
        def body(dir):
            ...
            Path(".zshrc").write_text(
                'PS1=""; setopt complete_in_word; fpath=($ZDOTDIR/autoload $fpath)'
            )
            os.execvpe(
                "zsh",
                ["--noglobalrcs", "--interactive"],
                {"ZDOTDIR": dir, "COLUMNS": "1000"},
            )
        ...
        try:
            ...
            bracketed_pastes = re.findall(
                rb"\x1b\[\?2004h(.*?)\x1b\[\?2004l", output, re.DOTALL
            )
            bracketed_pastes = [bp for bp in bracketed_pastes if bp != b""]
            bracketed_pastes = [bp.decode() for bp in bracketed_pastes]
            bracketed_pastes = [
                bp for bp in bracketed_pastes if bp != "autoload compinit; compinit"
            ]
        finally:
            os.waitpid(pid, 0)
...
"""
    ...
    bracketed_pastes = get_autoloaded_completion_output(
        ...
        GRAMMAR,
        "cmd",
        "cmd prefix--good{}{}".format(LEFT_ARROW * 5, TAB).encode(),
    )
