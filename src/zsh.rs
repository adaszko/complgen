...
// Zsh uses *dynamic* scoping for local variables, even if declared with 'local', hence 'declare' is used as slightly less misleading (!)
// Under Zsh, by default, candidates filtering is performed implicitly by compadd (!)
    ...
    if needs_compadds_code {
        ...
        if [[ -v "subword_compadd_transitions[$subword_state]" ]]; then
            ...
            for cmd_id in "${{(k)state_commands}}"; do
                ...
                IPREFIX="$matched_prefix" PREFIX="$subword" compadd_hook_swallow=true _{command}_cmd_${{cmd_id}}
                if [[ "$compadd_original_type" == builtin ]]; then
                    unfunction compadd
                else
...
fn write_match_transitions<W: Write>(
    ...
    if let Some(compadd_transitions) = &tables.compadd {
        writeln!(buffer, r#"    declare -A {prefix}compadd_transitions=()"#)?;
        for (state, state_transitions) in compadd_transitions {
            let transitions = state_transitions
                .iter()
                .map(|(cmd_id, to)| format!("[{cmd_id}]={}", to + ARRAY_START))
                .join(" ");
            writeln!(
                buffer,
                r#"    {prefix}compadd_transitions[{}]="({transitions})""#,
                state + ARRAY_START
            )?;
        }
    }
    ...
    if needs_top_level_compadds_code {
        ...
        if [[ -v "compadd_transitions[$state]" ]]; then
            ...
            for cmd_id in "${{(k)state_commands}}"; do
                ...
                functions -c compadd_hook compadd
                CURRENT=$word_index PREFIX="$word" compadd_hook_swallow=true _{command}_cmd_${{cmd_id}}
                if [[ "$compadd_original_type" == builtin ]]; then
                    unfunction compadd
                else
                    functions -c compadd_original compadd
