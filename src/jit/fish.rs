...
use crate::{dfa::DFA, StateId, aot::fish::write_subword_fn, regex::Input, grammar::{Specialization, DFARef}};
    ...
    for group in groups {
        for (literal, description) in group.iter().filter_map(|t| match t {
            ...
            if !description.is_empty() {
                writeln!(output, r#"    echo {}\t{}"#, literal, description)?;
            }
            else {
                writeln!(output, r#"    echo {}"#, literal)?;
            }
        }
        ...
        for cmd in group.iter().filter_map(|t| match t {
            ...
            writeln!(output, r#"    {fn_name} "$COMP_WORDS[$COMP_CWORD]""#)?;
        }
        ...
        for subdfa in group.iter().filter_map(|t| match t {
            ...
            writeln!(output, r#"    {} complete "${{words[$CURRENT]}}""#, make_subword_function_name(completed_command, *subdfa_id))?;
        }
        for cmd in group.iter().filter_map(|t| match t {
