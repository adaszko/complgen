...
use complgen::grammar::{to_railroad_diagram, to_railroad_diagram_file, Grammar, ValidGrammar};
...
fn handle_validation_error(g: Grammar, input: &str) -> anyhow::Result<ValidGrammar> {
    match ValidGrammar::from_grammar(g) {
        ...
        Err(Error::CommandAtNonTailPosition(_, complgen::grammar::ChicSpan::Dummy)) => {
            unreachable!()
        }
        Err(e) => {
            eprintln!("{}", e.to_string());
