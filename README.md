...
To address the problems with `{{{ ... }}}` positioning listed above, in the caveats section, it's possible to
specify a shell regex the command output has to match (the 2nd `foo` being the regex):

```
cmd ({{{ echo foo }}}@bash"foo" | bar);
```

With that additional information, the listed restrictions on `{{{ ... }}}` positioning are lifted and the
grammar compiles again.  Analogously for other problematic examples listed above.
